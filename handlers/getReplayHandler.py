    ...
    def asyncGet(self):
        try:
            ...
            if replayData == None:
                ...
                if replayData is None:
                    replayData = glob.db.fetch("SELECT ap_scores.*, users.username AS uname FROM ap_scores LEFT JOIN users ON ap_scores.userid = users.id WHERE ap_scores.id = %s", [replayID])
                    fileName = "{}_autopilot/replay_{}.osr".format(glob.conf.config["server"]["replayspath"], replayID)
                    UsingRelax2 = True
                else:
            ...
            # Serve replay
            log.info("[{}] Serving replay_{}.osr".format("RELAX" if UsingRelax else "AUTOPILOT" if UsingRelax2 else "VANILLA", replayID))
