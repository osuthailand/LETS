...
class LwScore:
    ...
    def __init__(self, score_id: Optional[int]=None, pp: Optional[int]=None, score_: Optional[scoreRelax2.score]=None):
        """
...
class SimpleRecalculator(Recalculator):
    ...
    def __init__(
        ...
        * str (one condition)
        ...
        q = "SELECT {} FROM ap_scores JOIN beatmaps USING(beatmap_md5) WHERE {} ORDER BY ap_scores.id DESC"
        super(SimpleRecalculator, self).__init__(
            ids_query=RecalculatorQuery(q.format("ap_scores.id AS id", conditions_str), parameters),
...
class ScoreIdsPool:
    ...
    def chunk(self, chunk_size: int) -> List[int]:
        ...
        self.logger.debug("Chunked {} ap_scores. Current scores in pool: {}".format(chunk_size, len(self.scores)))
...
class Worker:
    """
    A tomejerry worker. Recalculates pp for a set of ap_scores.
    """
    ...
    def recalculate_pp(self):
        ...
        if db_worker is None:
            self.logger.warning("Cannot fetch ap_scores. No database worker available!!")
            return
        ...
        try:
            ...
            for i, lw_score in enumerate(self.scores):
                ...
                cursor.execute(
                    "SELECT * FROM ap_scores JOIN beatmaps USING(beatmap_md5) WHERE ap_scores.id = %s LIMIT 1",
                    (lw_score.score_id,)
                )
    ...
    def save_recalculations(self):
        ...
        for i, lw_score in enumerate(self.scores):
            ...
            glob.db.execute("UPDATE ap_scores SET pp = %s WHERE id = %s LIMIT 1", (lw_score.pp, lw_score.score_id))
...
def mass_recalc(recalculator: Recalculator, workers_number: int=MAX_WORKERS, chunk_size: Optional[int]=None):
    ...
    FAILED_SCORES_LOGGER.addHandler(
        logging.FileHandler("tomejerry-relax2_failed_scores_{}.log".format(time.strftime("%d-%m-%Y--%H-%M-%S")))
    )
...
def main():
    ...
    recalc_group.add_argument(
        "-bh", "--hash", help="calculates pp for high scores played on a specific beatmap hash (beatmap_md5)", required=False
    )
    recalc_group.add_argument(
    ...
    # Get recalculator
    recalculators_gen = {
        "zero": lambda: SimpleRecalculator(("ap_scores.completed = 3", "pp = 0")),
        "recalc": lambda: SimpleRecalculator(("ap_scores.completed = '3'",)),
        "mods": lambda: SimpleRecalculator(("ap_scores.completed = 3", "mods & %s > 0"), (args.mods,)),
        "id": lambda: SimpleRecalculator(("ap_scores.id = %s",), (args.id,)),
        "gamemode": lambda: SimpleRecalculator(("ap_scores.completed = 3", "ap_scores.play_mode = %s",), (args.gamemode,)),
        "userid": lambda: SimpleRecalculator(("ap_scores.completed = 3", "ap_scores.userid = %s",), (args.userid,)),
        "beatmapid": lambda: SimpleRecalculator(("ap_scores.completed = 3", "beatmaps.beatmap_id = %s",), (args.beatmapid,)),
        "hash": lambda: SimpleRecalculator(("ap_scores.completed = 3", "beatmaps.beatmap_md5 = %s",), (args.hash,)),
        "fixstdhd": lambda: SimpleRecalculator(("ap_scores.completed = 3", "ap_scores.play_mode = 0", "ap_scores.mods & 8 > 0"))
    }
    recalculator = None
