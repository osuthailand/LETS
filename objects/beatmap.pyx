...
class beatmap:
	...
	def addBeatmapToDB(self):
		...
		if glob.conf.extra["mode"]["rank-all-maps"]:
			self.rankedStatus = 2
	...
	def saveFileName(self, fileName):
		...
		r = glob.db.fetch("SELECT file_name FROM beatmaps WHERE beatmap_md5 = %s LIMIT 1", (self.fileMD5,))
		if r is None:
			return
	...
	def setDataFromOsuApi(self, md5, beatmapSetID):
		...
		# Can't find beatmap by MD5. The beatmap has been updated. Check with beatmap set ID
		if mainData is None:
