...
def buildFullReplay(scoreID=None, scoreData=None, rawReplay=None):
    ...
    if scoreData is None:
        scoreData = glob.db.fetch(
            "SELECT ap_scores.*, users.username FROM ap_scores LEFT JOIN users ON ap_scores.userid = users.id "
            "WHERE ap_scores.id = %s",
            [scoreID]
        )
    else:
    ...
    if rawReplay is None:
        ...
        fileName = "{}_autopilot/replay_{}.osr".format(glob.conf.config["server"]["replayspath"], scoreID)
